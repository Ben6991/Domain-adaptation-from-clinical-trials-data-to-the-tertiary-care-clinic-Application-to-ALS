...
MODEL_PATH = folder + f'Exp2/{model_name}/models/lstm_model.model'
...
TAMC = TAMC[['ID', 'time', 'no'] + features + ['y']]
...
# ----- Scaling -----
features_to_scale = temporal_features + ['AgeOnset', 'Age', 'TimeSinceOnset', 'time']
patients_to_fit_scaler = train_pats.tolist()
scaler = MinMaxScaler((1, 2))
scaler.fit(PROACT.loc[PROACT['ID'].isin(patients_to_fit_scaler), features_to_scale])
preprocessed_data = PROACT.copy()
...
for number_iter in range(60):
    ...
    TAMC_scaled = TAMC.copy()
    TAMC_scaled.loc[:, features_to_scale] = scaler.transform(TAMC_scaled.loc[:, features_to_scale])

    X_test, y_test, lengths_test = ts.create_temporal_Xy_lists(TAMC_scaled, features, val_dict, True, 1, True, t_pred=True, time_to_next=True)
