...
def get_tasmc_test():
    temporal_data_file = '../Exp2/data/tasmc_temporal/tasmc_temporal_scaled_by_proact.pkl'
...
def train_transformer(train_data, early_data, early_stopping_kws, config):
    ...
    print("load pre-trained weights...", end=" ", flush=True)
    model.model.load_state_dict(torch.load(init_model_path))
    print("loaded.")
...
def fit_temporal(config, model_path):
    # get data
...
def load_flattened_data():
    data_file = '../Exp2/data/tasmc_static/tasmc_flat.csv'
...
def fit_static(config, model_path):
    # set early stopping for XGB and mlp
    X_train, y_train, X_early, y_early, X_test, y_test = load_flattened_data()
...
def get_config():
    return pickle.load(open("../Exp1/f18_best_configs.pkl", "rb"))["proact"][modelnm][permutation]
...
def fit_predict():
    model_path = f'results/{m_code(modelnm)}1/{permutation}.model'
    preds_path = f'results/{m_code(modelnm)}2/{permutation}.csv'

    config = get_config()
    print("Config: ", config)

    if is_static_model():
        preds = fit_static(config, model_path)
    else:
        preds = fit_temporal(config, model_path)
...
if __name__ == '__main__':
    ...
    modelnm = 'rnn'
    permutation = 0
    init_model_path = f'../Exp2/results/{d_code("proact")}{m_code(modelnm)}1/{permutation}.model'
    fit_predict()
